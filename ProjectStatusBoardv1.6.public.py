...
#        [Projects]
#    3 - Misc Accessory & Signs
#        [Projects]
#    4 - Pools, Fences, Solar, Trades
#        [Projects]
#
# 9- PR Assignments
#    [ReviewerName]
...
#
# Folder Structure - "[BVNumber] - [Note] [Address] - [City]"
...
column_names = ['Status', 'Created', 'Updated', 'BV Number', 'Address', 'City', 'Project Type', 'Reviewer']
df = pd.DataFrame(columns = column_names)
df.set_index('BV Number', inplace=True)

#replace path !!
logging.basicConfig(filename='path\\ProjectStatusBoardErrors.log', level=logging.ERROR, format='%(asctime)s %(levelname)s %(name)s %(message)s')
...
def savefile(msg): #saves dataframe as .csv file with datetime and message at end of filename
    ...
    #replace path !!
    fpname = 'path\\ProjectStatusBoardBackup ' + dt.strftime('%m-%d-%Y %H%M') + msg + '.csv'    #make filename
...
def board_status_booting(): #update status cell with "Booting..."
    sh = gc.open('Project Status Board') 
    wks = sh.worksheet_by_title('Comments') 
    wks.update_value('B2','Booting...')

def board_status_error(): #update status cell with "ERROR"
    sh = gc.open('Project Status Board') 
    wks = sh.worksheet_by_title('Comments') 
    wks.update_value('B2','ERROR')

def board_status_rebooting(): #update status cell with "REBOOTING"
    sh = gc.open('Project Status Board') 
    wks = sh.worksheet_by_title('Comments') 
    wks.update_value('B2','REBOOTING')

def get_df_from_google():   #get the old/current Assigned projects from the spreadsheet
    sh = gc.open('Project Status Board')
    wks2 = sh.worksheet_by_title('Assigned Residential')
...
def get_pending_from_google():  #get old/current Pending projects from spreadsheet, including checkboxes
    sh = gc.open('Project Status Board')
    wks = sh.worksheet_by_title('Pending Residential')
    dfg_pend = wks.get_as_df(index_column=1)
    return (dfg_pend)

def board_status_online():  #update status cell with "Online"
    sh = gc.open('Project Status Board')   
    wks = sh.worksheet_by_title('Comments') 
    wks.update_value('B2','Running')

def board_status_offline(): #update status cell with "Offline"
    sh = gc.open('Project Status Board')  
    wks = sh.worksheet_by_title('Comments') 
    wks.update_value('B2','Offline')
...
def get_fol_parts(Proj_Fol):    #extracts project info from folder name, returns BV number, address, city
    ...
    try:
        ...
        P_Addr = '' #float("NaN")
    try:
        P_City = (P_Parts[3])   #Project City
    except: #if can't find city due to delimiter error, or if other error, leave blank
        P_City = '' #float("NaN")

    return(BV_Num,P_Addr,P_City)    #returns the address, city, and BV number
...
def get_path_parts(dpath):  #extracts project info from folder path, returns project type, status, reviewer
    ...
    NewRev = "1. NEW RESIDENTIAL EPR"
    Assigned = "2. Residential PR Assignments"
    ...
    if NewRev in dirps: #if in "New EPR" folder
        ...
        elif re.search("Approve", dirps, re.IGNORECASE):    #if approved
            P_Stat = "Approved"
            P_Rev = GParFol #reviewer is grandparent folder
        else:   #otherwise
            P_Stat = "In Review"
...
def get_dataframe(dpath):   #puts project info in dataframe, returns project dataframe and folder name
    ...
    BV_Num,P_Addr,P_City = get_fol_parts(Proj_Fol)  #calls function to get parts from folder name
    ...
    NProj_Info = {'Status':[P_Stat],'Created':[CR_Time],'Updated':[UPD_Time],'BV Number':[BV_Num],'Address':[P_Addr],'City':[P_City],'Project Type':[P_Type],'Reviewer':[P_Rev]}
...
#    df_p = add_cols(df_p)   #adds cols
    dfpg.drop(['Status', 'Created', 'Updated', 'Address', 'City', 'Reviewer'],axis=1,inplace=True)
...
def initdatagrab(): #initial search through folders for projects. returns df
    ...
    for root, dirs, files in os.walk(Assig_Dir):      #walk through each folder in assignments directory, same as above
        ...
        if re.match("20..\-", Proj_Fol):    #if folder begins with BV number
            search_and_update(dpath)
    dfg.drop(['Status', 'Created', 'Updated', 'Address', 'City', 'Reviewer'],axis=1,inplace=True)
...
def update_google():    #update google sheet
    ...
    wks1 = sh.worksheet_by_title('Unassigned Residential') #select worksheet 1
    wks2 = sh.worksheet_by_title('Assigned Residential')   #select worksheet 2
...
def update_pending(df_p):   #update the pending sheet on google
    sh = gc.open('Project Status Board')   #open workbook
    wks_pending = sh.worksheet_by_title('Pending Residential')
...
class Watcher:
    ...
    def run(self):
        ...
        board_status_online()   #call f(x) to update board status on google sheets
...
if __name__=="__main__":
    while True: #this should create a loop so the program does some things on an error and then restarts
        try:
            board_status_booting()  #set the status cell on google to "Booting..."
            ...
            board_status_error()    #change the status cell on google to "ERROR"
            w.stop()    #stop the watchdogs
            board_status_rebooting()    #change the status cell on google to "REBOOTING"
        else: break
